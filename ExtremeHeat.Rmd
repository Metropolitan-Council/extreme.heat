...
get a refernce point; acres < ref temp, acres @ ref temp, acres > ref temp
ggplot(aes(x = tempbin, y = acres, col = year)) +
geom_bar()

say we have FIVE reference points; calc mean and SD of ref point. 
areas within 1 sd of reference temp; +-1sd (at reference), -2sd (cooler), +2sd (hotter)


```{r}
# On May 31, 2003, the Scan Line Corrector (SLC), which compensates for the forward motion of the satellite, failed.
metro_stations_daily
...
Details on how often we get pictures of our area. Landsats 8/9 circle earth every 16 days. This means that extreme heat events don't always line up with when we get satellite data. Prior to the launch of Landsat 8, there was a decade of years where landsat 7 operated with a scan line error (June 2003 - Feb 2013). 
  - Landsat specifics
    - The entirety of our region is covered by path 27, row 28-29. 
    - The east metro is also on path 26, and the west metro is also on path 28. 
