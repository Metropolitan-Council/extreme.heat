...
```{r include = F}
# library(tidyverse)
library(lubridate); library(dplyr); library(readr); library(ggplot2)
library(councilR)
library(tidycensus)
library(patchwork)
...
df1 <- filter(demographics, data == "percents") %>% full_join(heat) %>%
  ...
  mutate(name = str_replace(name, "% ", "total "),
         ...
         height = case_when(type == "Race" ~ 6,
                            TRUE ~ 3)) %>%
  filter(!is.na(name), variable != "income_below185pov_percent") %>%
  mutate(tempbin = case_when(heat2016 < 85 ~ "<85",
                             heat2016 < 90 ~ "85-89",
                             heat2016 < 95 ~ "90-94",
                             heat2016 < 100 ~ "95-99",
                             # heat2016 < 105 ~ "<105",
                             TRUE ~ "100+")) %>%
    mutate(tempbin = factor(tempbin, levels=c("100+", "95-99", "90-94", "85-89", "<85"))) %>% 
    arrange(tempbin) %>%
