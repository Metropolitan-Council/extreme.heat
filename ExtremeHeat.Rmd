...
df1 <- filter(demographics, data == "percents") %>% full_join(heat) %>%
  ...
  mutate(name = str_replace(name, "% ", "total "),
         ...
         height = case_when(type == "Race" ~ 6,
                            TRUE ~ 3)) %>%
  filter(!is.na(name)) %>% #, variable != "income_below185pov_percent") %>%
  mutate(tempbin = case_when(heat2022 < quantile(heat$heat2022, .2, na.rm = T) ~ 
                               paste0("<", round(quantile(heat$heat2022, .2, na.rm = T), 0)),
                             heat2022 < quantile(heat$heat2022, .4, na.rm = T) ~ 
                               paste0(round(quantile(heat$heat2022, .2, na.rm = T), 0) + 1, 
                                      "-", 
                                      round(quantile(heat$heat2022, .4, na.rm = T), 0)),
                             heat2022 < quantile(heat$heat2022, .6, na.rm = T) ~ 
                               paste0(round(quantile(heat$heat2022, .4, na.rm = T), 0) + 1, 
                                      "-", 
                                      round(quantile(heat$heat2022, .6, na.rm = T), 0)),
                             heat2022 < quantile(heat$heat2022, .8, na.rm = T) ~ 
                               paste0(round(quantile(heat$heat2022, .6, na.rm = T), 0) + 1, 
                                      "-", 
                                      round(quantile(heat$heat2022, .8, na.rm = T), 0)),
                             TRUE ~ 
                               paste0(round(quantile(heat$heat2022, .8, na.rm = T), 0) + 1, 
                                      "+"))) %>%
