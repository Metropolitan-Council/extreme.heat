...
acres_heat <- read_csv(
  ...
  select(-na) %>%# select(-`0`) 
  pivot_longer(names_to = "tempbin", values_to = "acres", -c(year, CTU_NAME)) %>%
  mutate(acres = as.numeric(acres)) %>%
  mutate(tempbin = factor(tempbin, levels=c("coolest", "cool", "average", "hot", "hottest")))
...
### Temperature 
- Google Earth Engine
- reference points, standard deviations
- statistical methods? 

### Demographics / other social variables
...
Indeed, certain populations have disproportionate exposure to extreme heat across our region. In particular: 
...
df1 <- filter(demographics, data == "percents") %>% full_join(heat) %>%
  ...
  mutate(tempbin = case_when(heat2016 < 85 ~ "<85",
                             ...
                             # heat2016 < 105 ~ "<105",
                             TRUE ~ "100+")) %>%
    mutate(tempbin = factor(tempbin, levels=c("<85", "85-89", "90-94", "95-99", "100+")))

heat_persons_plot <- function(x) {
 x %>% 
   group_by(variable, tempbin, name) %>%
...
Neither younger (under age 18) or older (65 or above) age groups who have increased sensitivity to extreme heat appear to be at an increased risk of exposure (Figure \@ref(fig:age-fig)). About 25% of the region's total population of a sensitive age lives in a block group with temperatures over 95 degrees.
