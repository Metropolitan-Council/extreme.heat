...
1) Do we want to get on a meeting schedule? Joel says yes, just pick a date.
  - COW meeting: big question is "with the Council's convening ability, could we pull together some shared definitions, consider developing a stakeholder group to guide this work? 
  - LUAC meeting: big question is "what tools/data would help you commit to and act on mitigating extreme heat? Thinking about resource allocations, planning decisions, etc.
  - Council Climate Workgroup: they are focusing on Climate Action Plan right now, may not be best fit
2) Move away from thinking about the hottest land temperature going up, and instead think about areas where heat risk has changed over time.
...
gee %>%
  ...
  labs( x = "Airport air temperature",
        ...
        col = "Landsat",
        caption = "Summer MSP airport land temperatures correlate with air temperatures.\nLandsat 7 data is only shown before the scan line error failure (pre 2003).") +
...
```{r}
#| fig.cap = "Regional temperature bins (relative to reference site temperatures). ",
#| results='markup', 
#| out.width= "3.3in",  
#| out.extra='style="float:right"', 
#| fig.align='right',
#| fig.asp = .3
#| 
knitr::include_graphics("./hottest_by_year/labeled/bin_2016.png") 
knitr::include_graphics("./hottest_by_year/labeled/bin_2021.png")
...
df1 <- filter(demographics, data == "percents") %>% full_join(heat) %>%
  ...
  mutate(name = str_replace(name, "% ", "total "),
         ...
         height = case_when(type == "Race" ~ 6,
                            TRUE ~ 3)) %>%
  filter(!is.na(name), variable != "income_below185pov_percent") %>%
  mutate(tempbin = case_when(heat2022 < 70 ~ "<70",
                             heat2022 < 75 ~ "70-74",
                             heat2022 < 80 ~ "75-79",
                             heat2022 < 85 ~ "80-84",
                             # heat2022 < 105 ~ "<105",
    ...
    arrange(tempbin) %>%
   group_by(type, variable, tempbin, name) %>%
...
heat_persons_plot <- function(x) {
 # filter(df1, type == "Age")  %>%
  ...
  geom_text(aes(label = if_else(lab_y <.05, NA_character_, label), y = lab_y),position = "stack", vjust = 1.5,
        size = 3) +
    labs(y = "", x = "", fill = "Temperature", title = "Regional totals",
         caption = str_wrap(paste0(first(x$source)), width = 60)) 
}
```
...
(filter(df, type == "Communications",
        name != "Communications; % accessing internet via cellphone only") %>% 
  heat_demo_plot() +
  scale_y_continuous(labels = scales::percent)) /

filter(df1, type == "Communications",
       # name != "Communications; total without internet at home",
       name != "Communications; total accessing internet via cellphone only") %>% 
  heat_persons_plot() +
  scale_y_continuous(labels = scales::percent)
```
